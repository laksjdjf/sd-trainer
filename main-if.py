...
def main(config):
    ...
    tokenizer = T5Tokenizer.from_pretrained(config.model.input_path, subfolder='tokenizer')
    tokenizer.model_max_length = 77
    text_encoder = T5EncoderModel.from_pretrained(config.model.input_path, subfolder='text_encoder')
    vae = AutoencoderKL.from_pretrained("waifu-diffusion/wd-1-5-beta2", subfolder='vae')
    vae.enable_slicing()
    ...
    noise_scheduler = DDPMScheduler.from_pretrained(config.model.input_path, subfolder='scheduler', variance_type = "fixed_small")
    ...
    for epoch in range(0, config.train.epochs, config.feature.minibatch_repeat):
        for batch in dataloader:
            ...
            if 'latents' in batch: # SDとの互換性のためIFの場合はpixel-valuesだがlatentsとしている。
