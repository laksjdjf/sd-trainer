...
import torch
from utils.functions import save_sd
...
diff2sd_map = {v: k for k, v in sd2diff_map.items()}

def replace_key(key, map):
...
class ControlNetLLLite(torch.nn.Module):
                ...
                if module.__class__.__name__ in target_replace_modules:
                    for child_name, child_module in module.named_modules():
                        ...
                        if is_linear or (is_conv2d and not SKIP_CONV2D):
                            ...
                            lllite_name = replace_key(lllite_name, diff2sd_map)
                            ...
                            if "emb_layer" in lllite_name or (
    ...
    def prepare_optimizer_params(self, text_lr, unet_lr):
        ...
        return [{"params":self.parameters(), "lr": unet_lr}]
...
if __name__ == "__main__":
    ...
    print("create unet")
    unet = UNet2DConditionModel.from_pretrained("/notebooks/sd-xl/",subfolder="unet")
    ...
    for step in range(steps):
        ...
        pooled = torch.randn(batch_size, 1280).cuda()
        size_cond = torch.tensor([1024,1024,0,0,1024,1024]*batch_size).cuda()
        added_cond_kwargs = {"text_embeds": pooled, "time_ids": size_cond}

        with torch.cuda.amp.autocast(enabled=True):
            control_net.set_cond_image(conditioning_image)

            output = unet(x, t, ctx, added_cond_kwargs=added_cond_kwargs).sample
    ...
    save_file(control_net.state_dict(), "./trained/controlnet/control_net.safetensors")