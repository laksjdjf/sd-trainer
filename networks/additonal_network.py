#参考：https://github.com/kohya-ss/sd-webui-additional-networks/blob/main/scripts/additional_networks.py
#応急処置的対応であり、元のコードの思想にはそぐわない
...
from scripts import lora_compvis , eh_compvis, model_util, metadata_editor, xyz_grid_support
    ...
    with gr.Group():
      with gr.Accordion('Additional Networks', open=False):
        ...
        for i in range(MAX_MODEL_COUNT):
          with gr.Row():
            module = gr.Dropdown(["LoRA","EH"], label=f"Network module {i+1}", value="LoRA")
    ...
    if models_changed:
      ...
      for module, model, weight_unet, weight_tenc in self.latest_params:
        ...
        if module == "EH":
          ...
          network, info = eh_compvis.create_network_and_apply_compvis(du_state_dict, weight_unet, unet)
          # in medvram, device is different for u-net and sd_model, so use sd_model's
          network.to(p.sd_model.device, dtype=p.sd_model.dtype)
          print(f"EH model {model} loaded: {info}")
          self.latest_networks.append((network, model))  
      if len(self.latest_networks) > 0:
