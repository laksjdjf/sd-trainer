...
class ReferenceTrainer(BaseTrainer):
    ...
    def loss(self, batch):
        ...
        with torch.autocast("cuda", dtype=self.autocast_dtype):
            with self.network.set_temporary_multiplier(self.alternate):
                model_output_w = self.diffusion(noisy_latents_w, timesteps, text_output_w, sample=False)
            with self.network.set_temporary_multiplier(0.0), torch.no_grad():
                model_output_l = self.diffusion(noisy_latents, timesteps.repeat(2), text_output, sample=False)[1:]
